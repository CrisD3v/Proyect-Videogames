    ...
    switch (action.type) {
        ...
        case ALPHABETICAL_SORT:
            ...
            if (action.payload === "default") {
                if (state.videogamesByName.length) alphabeticalSort = [...state.videogamesByName];
        ...
        case SORT_BY_RATING:
            let ratingSort;
            if (action.payload === "default") {
                if (state.videogamesByName.length) ratingSort = [...state.videogamesByName];
                else ratingSort = [...state.allVideogames];
            } 
            else {
                ratingSort =
                    action.payload === "low"? [...state.videogames].sort((a, b) => {
            ...
            if (!Array.isArray(action.payload)) {
                ...
                if (videogameInfo.genres.length) {
                    videogameInfo.genres = 
                        typeof videogameInfo.genres[0] !== "string" ? videogameInfo.genres.map((g) => g.name).join(", ")
